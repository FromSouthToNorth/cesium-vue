...
onMounted(() => {
  ...
  tileset.then((re) => {
    viewer.scene.primitives.add(re)
    ...
    GeoJsonDataSource.load(ranPolygon, {
      ...
      viewer.dataSources.add(dataSource).then((entity) => {
        entity.entities.values.forEach(e => {
          e.polygon.extrudedHeight = 200
        })
      })
    })

    // 获取实体位置并贴地
    async function getEntityPositionWithTerrain(entity, terrainProvider, time = JulianDate.now()) {
      if (!entity) {
        console.warn('实体为空');
        return null;
      }
      if (entity.position) {
        ...
        try {
          const terrainHeight = await sampleTerrainMostDetailed(terrainProvider, [cartographic]);
          if (terrainHeight[0].height !== undefined) {
            const _position = Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, terrainHeight[0].height);
            entity.position = _position;
            return _position;
          }
          console.warn(`无法获取实体 ${entity.id} 的地形高度，使用原始高度`);
          return position;
        } catch (error) {
          console.error(`查询地形高度失败（实体 ${entity.id}）:`, error);
          return position;
        }
      }
    ...
    async function processPointsAndDistances(pointGeoJSON, pointGeoJSONs) {
      try {
        ...
        const basePosition = await getEntityPositionWithTerrain(basePoint, viewer.terrainProvider);
        if (!basePosition) {
        ...
        for (const entity of entities) {
          const targetPosition = await getEntityPositionWithTerrain(entity, viewer.terrainProvider);
